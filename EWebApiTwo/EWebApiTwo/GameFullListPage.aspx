...
<body>
    <form id="form1" runat="server">
        <div>
            Full Game List</div>
        <asp:GridView ID="GridView1" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" DataKeyNames="Expr1" DataSourceID="SqlDataSource2" CellPadding="4" ForeColor="#333333" GridLines="None">
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:LoginConnectionString %>" DeleteCommand="DELETE FROM [Game] WHERE [ID] = @original_ID AND [Title] = @original_Title AND [RelaseDate] = @original_RelaseDate AND [Img] = @original_Img AND [BackgroundImg] = @original_BackgroundImg AND (([Platform_ID] = @original_Platform_ID) OR ([Platform_ID] IS NULL AND @original_Platform_ID IS NULL)) AND [Developer_ID] = @original_Developer_ID" InsertCommand="INSERT INTO [Game] ([Title], [RelaseDate], [Img], [BackgroundImg], [Platform_ID], [Developer_ID]) VALUES (@Title, @RelaseDate, @Img, @BackgroundImg, @Platform_ID, @Developer_ID)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT * FROM [Game]" UpdateCommand="UPDATE [Game] SET [Title] = @Title, [RelaseDate] = @RelaseDate, [Img] = @Img, [BackgroundImg] = @BackgroundImg, [Platform_ID] = @Platform_ID, [Developer_ID] = @Developer_ID WHERE [ID] = @original_ID AND [Title] = @original_Title AND [RelaseDate] = @original_RelaseDate AND [Img] = @original_Img AND [BackgroundImg] = @original_BackgroundImg AND (([Platform_ID] = @original_Platform_ID) OR ([Platform_ID] IS NULL AND @original_Platform_ID IS NULL)) AND [Developer_ID] = @original_Developer_ID">
            <DeleteParameters>
